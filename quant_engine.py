...
from scipy.stats import multivariate_t, norm, t, chi2
...
class QuantitativeEngine:
    ...
    def run_monte_carlo(self, weights, time_horizon=30, initial_investment=10000, num_simulations=10000):
        ...
        worst_cases = simulation_results[simulation_results <= percentile_5]
        cvar_95 = initial_investment - np.mean(worst_cases)

        print(f"95% Value at Risk (VaR): ${var_95:.2f}")
        print(f"95% Conditional Value at Risk (CVaR): ${cvar_95:.2f}")
        return simulation_results, var_95, cvar_95